...
class Canvas(app.Canvas):
    def __init__(self):
        app.Canvas.__init__(self, title='UART',
                            keys='interactive')
    ...
    def on_mouse_wheel(self, event):
        ...
        scale_x_new, scale_y_new = (scale_x * math.exp(x_scale*dx),
                                    scale_y * math.exp(y_scale*dx))
    ...
    def on_timer(self, event):
        ...
        if not paused :
            ...
            if sum(data[0:int(k/2)*2:2]) < sum(data[1:int(k/2)*2:2]):
                y[:, -int(k/2):] = (1/2048)*(data[0:int(k/2)*2:2]*256+data[1:int(k/2)*2:2])
            else:
                y[:, -int(k/2):] = (1/2048)*(data[1:int(k/2)*2:2]*256+data[0:int(k/2)*2:2])
